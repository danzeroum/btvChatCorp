...
async fn update(
    ...
    .bind(id).bind(auth.workspace_id)
    .bind(&dto.name).bind(&dto.description).bind(&dto.icon).bind(&dto.color)
    .bind(&dto.status).bind(&dto.category).bind(&dto.priority)
...
async fn remove(
    ...
    if r.rows_affected() == 0 { return Err(AppError::not_found("Projeto nao encontrado")); }
...
async fn list_instructions(
    ...
    let rows: Vec<(Uuid, String, Option<String>, String, String, bool, chrono::DateTime<chrono::Utc>)> =
        sqlx::query_as(
    ...
    Ok(Json(rows.into_iter().map(|(id,name,desc,content,trigger,active,created)| serde_json::json!({
