...
async fn list(
    ...
    let rows = sqlx::query_as::<_, Document>(
        r#"
        SELECT id, workspace_id, filename, original_filename, mime_type,
...
async fn upload(
    ...
    tokio::fs::create_dir_all(&upload_path)
        .await
        .map_err(|_| AppError::internal("Erro ao criar diretório de uploads"))?;

    while let Some(field) = multipart.next_field().await
        .map_err(|_| AppError::bad_request("Erro no upload"))? 
    {
        ...
        let data = field.bytes().await
            .map_err(|_| AppError::bad_request("Erro ao ler arquivo"))?;
        ...
        tokio::fs::write(&dest, &data)
            .await
            .map_err(|_| AppError::internal("Erro ao salvar arquivo"))?;
...
async fn remove(
    ...
    if r.rows_affected() == 0 { return Err(AppError::not_found("Documento não encontrado")); }
...
async fn list_for_project(
    ...
    sqlx::query!("SELECT id FROM projects WHERE id=$1 AND workspace_id=$2", project_id, auth.workspace_id)
        .fetch_one(&state.db).await
        .map_err(|_| AppError::not_found("Projeto não encontrado"))?;

    let rows = sqlx::query_as::<_, Document>(
        r#"
        SELECT d.id, d.workspace_id, d.filename, d.original_filename, d.mime_type,
...
async fn link_to_project(
    ...
    sqlx::query!(
        r#"
        INSERT INTO project_documents (project_id, document_id, linked_by)
        VALUES ($1,$2,$3) ON CONFLICT DO NOTHING
        "#,
        project_id, dto.document_id, auth.user_id,
    )
    .execute(&state.db).await?;
