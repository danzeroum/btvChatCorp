...
@Component({
        ...
        @for (session of sessions(); track session.id) {
          <div class="session-card" [class.current]="session.isCurrent">
            ...
            <div class="session-details">
              <span>&#127760; <code>{{ session.ip }}</code></span>
  ...
  styles: [`
    ...
    .active-sessions { padding: 28px 32px; background: var(--panel-2); min-height: 100vh; }
    ...
    .session-card { background: var(--white); border:1px solid var(--line); border-radius:12px; padding:20px 24px; display:flex; flex-direction:column; gap:12px; }
    .session-card.current { border-color: var(--acc); background: var(--panel); }
    .session-user { display:flex; align-items:center; gap:12px; }
    .avatar-sm { display:inline-flex; align-items:center; justify-content:center; width:36px; height:36px; border-radius:50%; background: var(--acc-soft); color: var(--acc); font-size:13px; font-weight:600; flex-shrink:0; }
    .session-user strong { display:block; font-size:14px; font-weight:600; color: var(--ink); }
    .session-email { font-size:12px; color: var(--ink-2); }
    ...
    .session-details { display:flex; flex-wrap:wrap; gap:12px; font-size:12px; color: var(--ink-2); }
    .session-details code { font-family:'IBM Plex Mono',monospace; font-size:11px; background:var(--panel-2); padding:1px 5px; border-radius:3px; }
