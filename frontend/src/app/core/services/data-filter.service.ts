...
import { WorkspaceContext, FilteredMessage, PIIDetection, DataClassification } from '../../shared/models/data-classification.model';
...
export class DataFilterService {
  ...
  private readonly piiTypeNames = ['CPF', 'CNPJ', 'EMAIL', 'PHONE', 'CREDIT_CARD'] as const;
  ...
  private classifyContent(text: string, ctx: WorkspaceContext): DataClassification {
    const keywords = ctx.sensitiveKeywords ?? [];
    const hasKeyword = keywords.some((kw: string) => text.toLowerCase().includes(kw.toLowerCase()));
    return {
